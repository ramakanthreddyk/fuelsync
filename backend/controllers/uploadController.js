...
const uploadFile = async(req, res) => {
    try {
        ...
        // OCR Processing
        const visionEndpoint = process.env.AZURE_VISION_ENDPOINT + 'vision/v3.2/read/analyze';
        ...
        for (let i = 0; i < 10; i++) {
            ...
            if (result.data.status === 'succeeded') {
                ocrResult = result.data.analyzeResult.readResults.flatMap(r => r.lines.map(line => line.text));
                console.log('[OCR] Extracted text lines:', ocrResult);
                break;
            }
            await new Promise(resolve => setTimeout(resolve, 2000));
        }
        if (!ocrResult) return res.status(500).json({ message: 'OCR failed or timed out' });
        ...
        for (const line of ocrResult) {
            if (!date) {
                ...
                if (match) date = match[0].split('/').reverse().join('-');
            }
            if (!timeStr) {
        ...
        for (let i = 0; i < ocrResult.length; i++) {
            ...
            const nozzleMatch = line.match(/nozzle\s*no?\s*([0-9a-zA-Z]+)/) || line.match(/no\s*([0-9a-zA-Z]+)/);
            if (nozzleMatch) {
                const nozzleRaw = nozzleMatch[1];
                const cleaned = nozzleRaw.replace(/[^0-9a-zA-Z]/g, '').toLowerCase();
                if (['nol', 'no1', 'noi', 'l', '1'].includes(cleaned)) currentNozzle = 1;
            ...
            if (currentNozzle) {
                for (let j = i; j < Math.min(i + 5, ocrResult.length); j++) {
                    ...
                    if (nearby.includes('cumvolume')) {
                        const nextLine = ocrResult[j + 1]?.replace(/\s+/g, '').replace(/,/g, '');
                        const numMatch = nextLine?.match(/(\d+\.\d+)/);
                        if (numMatch) {
                            nozzles[currentNozzle] = parseFloat(numMatch[1]);
                            console.log(`[OCR] Nozzle ${currentNozzle} CumVolume: ${nozzles[currentNozzle]}`);
                        }
                    }
                }
            }
    ...
    `;
        const values = [
            uploadId, userId, blobName, file.originalname, 'uploaded', pumpSno, date, timeStr,
            nozzles[1], nozzles[2], nozzles[3], nozzles[4]
        ];
...
const getUploads = async(req, res) => {
    try {
        ...
        const result = await pool.query(
            `SELECT * FROM uploads WHERE user_id = $1 ORDER BY uploaded_at DESC`, [userId]
        );
